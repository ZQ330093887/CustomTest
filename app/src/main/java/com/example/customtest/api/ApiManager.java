...
import com.example.customtest.App;
import com.example.customtest.R;
import com.example.customtest.utils.NetworkUtils;
...
public class ApiManager {
    ...
    private static final Interceptor mRewriteCacheControlInterceptor = chain -> {
        ...
        if (!NetworkUtils.isConnected(App.getmInstance().getApplicationContext())) {
    ...
    private static OkHttpClient getOkHttpClient() {
        if (mOkHttpClient == null) {
            synchronized (ApiManager.class) {
                Cache cache = new Cache(new File(App.getmInstance().getApplicationContext().getCacheDir(), "HttpCache"), 1024 * 1024 * 100);
                if (mOkHttpClient == null) {
                        ...
                        if (e instanceof HttpException) {
                            ...
                            if (result != null) {
                                if (e.getMessage() == null) {
                                    result.error(App.getmInstance().getApplicationContext().getString(R.string.status_layout_manager_error));
                                } else {
