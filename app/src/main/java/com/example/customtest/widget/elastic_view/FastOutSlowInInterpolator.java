...
public class FastOutSlowInInterpolator implements Interpolator {
    ...
    public float getInterpolation(float input) {
        ...
        if (input >= 1.0F) {
            ...
            int position = Math.min((int) (input * (float) (_values.length - 1)), _values.length - 2);
            float quantized = (float) position * _centreColor;
            float diff = input - quantized;
            float weight = diff / _centreColor;
            var10000 = _values[position] + weight * (_values[position + 1] - _values[position]);
        }
